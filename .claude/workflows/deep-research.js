export const meta = {
  ...
  phases: [{"title":"Scope","detail":"Decompose question (from args) into 5 search angles","model":"opus"},{"title":"Search","detail":"5 parallel WebSearch agents, one per angle","model":"sonnet"},{"title":"Fetch","detail":"URL-dedup, fetch top 15 sources, extract falsifiable claims","model":"opus"},{"title":"Verify","detail":"3-vote adversarial verification per claim (need 2/3 refutes to kill)","model":"fable"},{"title":"Synthesize","detail":"Merge semantic dupes, rank by confidence, cite sources","model":"fable"}],
}

// deep-research (tiered override of the built-in workflow, shadows the same name):
// Model tiering is deliberate, not uniform. Standing rule for this repo: tier UP wherever a
// stage carries judgment or review weight, and only drop to a cheap model for stages that are
// genuinely mechanical. An all-cheap pipeline produces a report that reads confident and is not.
//   judge stages (Verify, Synthesize) = fable
//   normal reasoning (Scope, Fetch)   = opus
//   search-and-report (Search)        = sonnet
// Flow: Scope -> pipeline(Search -> URL-dedup -> Fetch+Extract) -> 3-vote Verify -> Synthesize
// WebSearch/WebFetch only. This workflow never reads or writes the repo.
...
const scope = await agent(
  ...
  { label: "scope", schema: SCOPE_SCHEMA, model: "opus" }
)
if (!scope) {
...
const report = await agent(
  ...
  { label: "synthesize", schema: REPORT_SCHEMA, model: "fable" }
)
