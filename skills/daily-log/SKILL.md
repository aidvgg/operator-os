...
4. Write today's planned checklist into `days/YYYY-MM-DD.md` (always create it when tasks are synced to the calendar, so the event ids are recorded; otherwise the table row alone is fine). Day-file shape, since a fresh repo ships no example: H1 `# YYYY-MM-DD (DOW, Www)`; `## Planned` as a checklist, one line per task in the form `- [ ] [tag] task (est Nh)` with the motion tag in backticks; `## Calendar (synced)` as a Block / Task / Event id table when anything was synced. `/eod` appends `## Shipped`, `## Not done` and `## Numbers` (focus hours by motion) under the same H1.
