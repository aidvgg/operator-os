# Fonts for the proposal generator

Three latin-subset variable woff2 files.
`skills/proposal-creator/scripts/generate-proposal-html.js` reads them off disk and inlines each
one as a base64 `@font-face` src, so a generated proposal renders identically on a machine that
...
| `JetBrainsMono-var-latin.woff2` | JetBrains Mono | labels, numbers, monospaced detail |
...
also ship under `skills/invoice-creator/assets/fonts/`, byte-identical. If the only licence copy
